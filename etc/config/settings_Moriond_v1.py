...
pt_var_name = 'probe_Ele_pt'
mass_var_name = 'mass'

if include_FSR :
    pt_var_name = probe_with_FSR_pt
    mass_var_name = ''

HZZ = """(probe_ele_Spring16_HZZ_WP == 1 && %s > 7.0 &&
                probe_Ele_sip < %f &&
                probe_Ele_correctediso < %f &&
                abs(probe_Ele_dxy) < %f &&
                abs(probe_Ele_dz)  < %f
                )""" % (pt_var_name, SIP_cut, ISO_cut, Dxy_cut, Dz_cut)
HZZ = HZZ.replace('\n', '')
# flag to be Tested
flags = {
    'passing_HZZ'  : HZZ,
    }
data_name = 'sum_rerecoBDEFG_promptHv23'
baseOutDir = 'results/Moriond17_v1_%s/'%data_name
...
samplesDef = {
    'data'   : tnpSamples.Moriond17_v1[data_name].clone(),
    'mcNom'  : tnpSamples.Moriond17_v1['DY_madgraph'].clone(),
    'mcAlt'  : None, #tnpSamples.Moriond17_v1['DY_NLO'].clone(),
    'tagSel' : tnpSamples.Moriond17_v1['DY_madgraph'].clone(),
}
...
biningDef = [
   ...
   { 'var' : pt_var_name , 'type': 'float', 'bins': [7, 15, 20, 30, 40, 50, 60, 70, 80, 100, 120, 140, 160, 200] },
]
...
#############################################################
### cut
cutBase   = 'tag_Ele_pt > 30 && abs(tag_sc_eta) < 2.1' #probe_ele_isGap == 0 &&vb 
